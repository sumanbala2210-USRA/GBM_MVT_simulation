    ...
    for campaign in config.get('simulation_campaigns', []):
        ...
        for pulse_config in campaign.get('pulses_to_run', []):
            ...
            for combo in itertools.product(*param_values):
                ...
                # Sanity Check: Ensure detector_selections is used correctly.
                if sim_type == 'gbm' and detector_selections:
                    # Get the detector setting from the simulation's parameters. 
                    base_dets = current_variable_params.get('trigger_set', {}).get('det')
                    
                    # If a flexible selection is requested, but the base data wasn't generated for 'all' detectors...
                    if base_dets not in ['all', ['all']]:
                        logging.error(
                            "\nXXXXXXXXXXXXXX Configuration Error: XXXXXXXXXXXXXXX\n"
                            f"'detector_selections' is active, but the simulation\n "
                            f"is configured to run only for det='{base_dets}', not 'all'. \n"
                            "XXXXXXXXXXXXXX Configuration Error: XXXXXXXXXXXXXXX" 
                        )
                ...
                if param_dir_path.exists():
                    for bin_width_ms in bin_widths_to_analyze_ms:
                        # =================== The New Logic ===================
                        # IF detector_selections is defined in the YAML, run in FLEXIBLE mode.
                        if detector_selections and sim_type == 'gbm':
                            for selection in detector_selections:
                                yield {
                                    ...
                                    'bin_width_to_process_ms': bin_width_ms,
                                    'dets_to_analyze': selection,
                                    'haar_python_path': haar_python_path
                                }
                        # ELSE, run in the original, SIMPLE mode.
                        else:
                            yield {
                                'param_dir_path': param_dir_path,
                                'base_params': {**current_variable_params, 'pulse_shape': pulse_shape, 'sim_type': sim_type, 'num_analysis': total_sim_analysis},
                                'analysis_settings': analysis_settings,
                                'bin_width_to_process_ms': bin_width_ms,
                                # Note: dets_to_analyze is not included in this case
                                'haar_python_path': haar_python_path
                            }
"""
    ...
    for campaign in config.get('simulation_campaigns', []):
        ...
        for pulse_config in campaign.get('pulses_to_run', []):
            ...
            if pulse_shape == 'complex_pulse':
                ...
                #    This now correctly uses only the fixed canonical parameters.
                template_generation_params = {'peak_amplitude': 1.0, 'position': 0.0}
                if 'trigger_set' in campaign.get('parameters', {}):
                ...
                for combo in itertools.product(*param_values):
                    ...
                    if base_dets == 'all' and not detector_selections:
                        logging.error(
                                "\nXXXXXXXXXXXXXX Configuration Error: XXXXXXXXXXXXXXX\n\n"
                                f"'detector_selections' is Needed as det = {base_dets}\n\n"
                                f"XXXXXXXXXXXXXXX Configuration Error: XXXXXXXXXXXXXXX"
                            )
                    sys.exit(1)

                    # Sanity Check for detector selections in complex mode
                    if detector_selections:
                    ...
                    for bin_width_ms in bin_widths_to_analyze_ms:
                        # Yield tasks with flexible detector selection if requested
                        if detector_selections:
                            for selection in detector_selections:
                                yield {'param_dir_path': template_dir_path,
                                       'base_params': {**current_variable_params, 'pulse_shape': pulse_shape, 'sim_type': sim_type, 'num_analysis': total_sim_analysis},
                                       'analysis_settings': analysis_settings,
                                       'bin_width_to_process_ms': bin_width_ms,
                                       'dets_to_analyze': selection,
                ...
                for combo in itertools.product(*param_values):
                    ...
                    if param_dir_path.exists():
                        for bin_width_ms in bin_widths_to_analyze_ms:
                            if detector_selections and sim_type == 'gbm':
                                for selection in detector_selections:
                                    yield {'param_dir_path': param_dir_path,
                                           'base_params': {**current_variable_params, 'pulse_shape': pulse_shape, 'sim_type': sim_type, 'num_analysis': total_sim_analysis},
                                           'analysis_settings': analysis_settings,
                                           'bin_width_to_process_ms': bin_width_ms,
                                           'dets_to_analyze': selection,
...
def main():
    ...
    # Use a bin width from the config for the run folder name, if available
    try:
        analysis_bin = config['analysis_settings']['bin_widths_to_analyze_ms'][0]
        run_folder_name = f"run_{e_n(analysis_bin)}_{now}"
    except (KeyError, IndexError):
        run_folder_name = f"run_{now}"

    # Create a new, timestamped directory for this run's results
    run_results_path = results_path / run_folder_name
    run_results_path.mkdir(parents=True, exist_ok=True)
    ...
    logging.info(f"Generated {len(tasks)} analysis tasks. Starting parallel processing.")
    
    all_results = []
    
    # Chunking and parallel processing logic (unchanged)
    ...
    # =================== NEW "SAVE FINAL SUMMARY" BLOCK ===================
    if all_results:
        # 1. Create the single master DataFrame from all collected results.
        final_df = pd.DataFrame(all_results)
        
        # 2. Save the Master CSV File (containing all results).
        master_csv_path = run_results_path / RESULTS_FILE_NAME
        final_df.to_csv(master_csv_path, index=False)
        logging.info(f"✅ Master summary with all results saved to:\n{master_csv_path}")

        # 3. Group the DataFrame and save individual, clean CSV files for each group.
        logging.info("Saving individual summary files for each group...")
        try:
            # Group by the simulation type and the pulse shape.
            grouped = final_df.groupby(['sim_type', 'pulse_shape'])
            for (sim_type, pulse_shape), group_df in grouped:
                
                # Create a clean filename for this specific group.
                group_filename = f"{sim_type}_{pulse_shape}_summary.csv"
                group_csv_path = run_results_path / group_filename
                
                # Drop columns that are completely empty for this group to make the CSV clean.
                group_df_cleaned = group_df.dropna(axis=1, how='all')
                
                group_df_cleaned.to_csv(group_csv_path, index=False)
                logging.info(f"  > Saved group summary to: {group_csv_path}")
        except Exception as e:
            logging.error(f"Could not save grouped CSV files. Error: {e}")
